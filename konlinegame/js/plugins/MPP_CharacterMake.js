...
/*:
 * @plugindesc 【ver.1.8】ゲーム内でキャラクター生成を行えます。
 * @author 木星ペンギン
 *
 * @help プラグインコマンド:
...
GenerateColor.prototype.load = function() {
    if (this._path && !this._image) {
        ...
        var url = `Generator/${this._path}.png`;
...
GenerateColor.prototype.decryptImg = function(url) {
    ...
    requestFile.onload = function () {
        if(this.status < Decrypter._xhrOk) {
            ...
            color._image.src = Decrypter.createBlobUrl(arrayBuffer);
            color._image.onload = GenerateColor.prototype._onLoad.bind(color);
            color._image.onerror = GenerateColor.prototype._onError.bind(color);
        }
    };
